        ...
        public State State
        {
            get
            {
                if (this._state == State.Unknown)
                {
                    ...
                    if (string.IsNullOrEmpty(this.Guid) || !StateStorage.Instance.Exists(key)) this.State = State.Fresh; // if key does not exists in state-storage already, then it's just fresh.                    
