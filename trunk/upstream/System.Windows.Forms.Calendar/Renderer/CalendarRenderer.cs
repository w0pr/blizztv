...
namespace System.Windows.Forms.Calendar
{
    ...
    public class CalendarRenderer
    {
        ...
        /// when in <see cref="DaysModes.Expanded"/>
        /// </summary>
        ...
        {
            get { return Calendar.DaysMode == DaysModes.Short; }
        }
        ...
        /// <summary>
        /// Gets if the TimeScale is currently visible.
        /// </summary>
        public bool TimeScaleVisible
        {
            get { return Calendar.DaysMode == DaysModes.Expanded; }
        }
        ...
        /// <summary>
        /// Gets the width of the header of weeks, whenCalendar.DaysMode in <see cref="DaysModes.Short"/>
        /// </summary>
        public virtual int WeekHeaderWidth
        {
            get
            {
                if (!Calendar.ShowWeekHeader)
                    return 0;
        ...
        /// This only works when Calendar.DaysMode is in <c>Expanded</c> mode.
        /// </summary>
        ...
        public GraphicsPath ItemRectangle(CalendarRendererItemBoundsEventArgs evtData, Rectangle bounds)
        {
            ...
            //    !evtData.Item.IsOnDayTop && evtData.Calendar.DaysMode == DaysModes.Expanded)
            //{
            //    /*
            //     * Trace pointed item
            //     * 
        ...
        /// Items the pattern.
        /// </summary>
        /// <param name="e">The <see cref="System.Windows.Forms.Calendar.CalendarRendererItemBoundsEventArgs"/> instance containing the event data.</param>
        ...
        /// <param name="drawDot"></param>
        public void ItemBorder(CalendarRendererItemBoundsEventArgs e, Rectangle bounds, Color color, float width, bool drawDot)
        {
            using (GraphicsPath r = ItemRectangle(e, bounds))
            {
        ...
        public void PerformLayout(bool performItemsLayout)
        {
            ...
            if (Calendar.DaysMode == DaysModes.Expanded)
            {
                ...
                for (int i = 0; i < Calendar.Days.Count; i++)
                {
                    ...
                    day.Bounds = new Rectangle(curLeft, curTop, dayWidth, dayHeight);
                    day.DayTop.Bounds = new Rectangle(curLeft, day.HeaderBounds.Bottom, dayWidth, DayTopHeight);
                    curLeft += dayWidth + 1;
                    //int k = 0;
                    ...
                    for (int j = 0; j < day.TimeUnits.Length; j++)
                    {
                        ...
                        bool showTimeUnit = (Calendar.TimeUnitsOffset * -1 < (j + 1));
                        unit.Visible = showTimeUnit;
                        ...
                        //}
                        unit.Bounds = new Rectangle(day.Bounds.Left, utop, day.Bounds.Width, TimeScaleUnitHeight);
            ...
            else
            {
                ...
                dayWidth = (Calendar.ClientSize.Width - 7 - leftStart) / 7;
                dayHeight = (Calendar.ClientSize.Height - curTop) / (Calendar.Days.Count / 7) - 1;
                _dayNameHeaderColumns = new Rectangle[7];
                int j = 0;

                for (int i = 0; i < Calendar.Days.Count; i++)
                {
                    Calendar.Days[i].Bounds = new Rectangle(curLeft, curTop, dayWidth, dayHeight);
                    curLeft += dayWidth + 1;
                    ...
                    {
                        Calendar.Weeks[j++].Bounds = new Rectangle(0, curTop, Calendar.ClientSize.Width, dayHeight);
                    }
                }
                #endregion
            }
        ...
        public void PerformItemsLayout()
        {
            ...
            if (Calendar.DaysMode == DaysModes.Expanded)
            {
                ...
                foreach (CalendarItem item in Calendar.Items)
                {
                    ...
                    if (item.IsOnDayTop)
                    {
                        ...
                        if (dayEnd == null) dayEnd = Calendar.Days[Calendar.Days.Count - 1];
                        ...
                        item.Bounds = Rectangle.FromLTRB(dayStart.DayTop.Bounds.Left, 0, dayEnd.DayTop.Bounds.Right, 1);
                ...
                int[,] tmatix = new int[Calendar.Days.Count, maxItemsOnDayTop];
                ...
                foreach (CalendarDay day in Calendar.Days)
                {
                    ...
                    foreach (List<CalendarItem> group in groups)
                    {
                        ...
                        foreach (CalendarItem item in group)
                        {
                            ...
                            item.Bounds = Rectangle.FromLTRB(rleft, rtop, right, bottom);
                            item.MinuteStartTop = GetTimeY(item.StartDate.TimeOfDay);
                            item.MinuteEndTop = GetTimeY(item.EndDate.TimeOfDay);
            ...
            else if (Calendar.DaysMode == DaysModes.Short)
            {
                ...
                for (int i = 0; i < Calendar.Days.Count; i++)
                {
                    ...
                    Calendar.Days[i].OverflowEnd = false;
                    Calendar.Days[i].OverflowStart = false;
                }
                ...
                int[,] matix = new int[Calendar.Days.Count, maxItems];
                int curIndex = 0;
                foreach (CalendarItem item in Calendar.Items)
                {
                    CalendarDay dayStart = item.DayStart;
                ...
                for (int week = 0; week < Calendar.Weeks.Length; week++)
                {
                    ...
                    foreach (CalendarItem item in Calendar.Items)
                    {
                        ...
                        if (r.Bottom <= sunday.Bounds.Bottom)
                            ...
                            for (int i = dayStart.Index; i <= dayEnd.Index; i++)
                                Calendar.Days[i].OverflowEnd = true;
        ...
        {
            if (Calendar.DaysMode == DaysModes.Short)
                return 0;
            else if (Calendar.Days != null && Calendar.Days.Count > 0)
            {
                ...
                double visibleTimeUnits = Calendar.Days[0].BodyBounds.Height / (int)TimeScaleUnitHeight;
                return (int)visibleTimeUnits;
            }
        ...
        public virtual void OnDrawTimeScale(CalendarRendererEventArgs e)
        {
            ...
            float hourFontSize = e.Calendar.Font.Size * (e.Calendar.TimeScale == TimeScales.SixtyMinutes ? 1f : 1.5f);
            Font hourFont = new Font(e.Calendar.Font.FontFamily, hourFontSize);
            ...
            //for (int i = this.Calendar.VScrollValue; i < e.Calendar.Days[0].TimeUnits.Length; i++)
            {
                ...
                if (!string.IsNullOrEmpty(minutes))
                {
                    ...
                    CalendarRendererBoxEventArgs hevt = new CalendarRendererBoxEventArgs(e, 
                        new Rectangle(hourLeft, unit.Bounds.Top, hourWidth, unit.Bounds.Height), 
                        hours, TextFormatFlags.Right);

                    hevt.Font = hourFont;
                    this.OnDrawTimeScaleHour(hevt);

                    if (k++ == 0 || unit.Hours == 0 || unit.Hours == 12) 
                        minutes = unit.Date.ToString("tt");

                    CalendarRendererBoxEventArgs mevt = new CalendarRendererBoxEventArgs(e, 
                        new Rectangle(minuteLeft, unit.Bounds.Top, minuteWidth, unit.Bounds.Height), 
                        minutes, TextFormatFlags.Top | TextFormatFlags.Left);

                    mevt.Font = minuteFont;
                    this.OnDrawTimeScaleMinutes(mevt);
                }
            }
        }
        ...
        {
            for (int i = 0; i < e.Calendar.Days.Count; i++)
            {
                CalendarDay day = e.Calendar.Days[i];

                e.Tag = day;
                CalendarRendererDayEventArgs cArgs = new CalendarRendererDayEventArgs(e, day);
                this.OnDrawDay(cArgs);
            }
        }
        ...
        public virtual void OnDrawDay(CalendarRendererDayEventArgs e)
        {
            ...
            if (Calendar.DaysMode == DaysModes.Short && (day.Index == 0 || day.Date.Day == 1))
                hevt.Text = day.Date.ToString("d MMM");
            

            OnDrawDayHeaderText(hevt, day.Date);

            if (devt.TextSize.Width < day.HeaderBounds.Width - hevt.TextSize.Width * 2
                && (e.Calendar.DaysMode == DaysModes.Expanded))
            {
                OnDrawDayHeaderText(devt, day.Date);
            }
        ...
        public virtual void OnDrawDayHeaderText(CalendarRendererBoxEventArgs e, DateTime atDate)
        {
            DrawStandarBoxText(e);
        }
        ...
        /// Calendar.DaysMode is <see cref="DaysModes.Short"/>
        /// </summary>
        /// <param name="e"></param>
        ...
        /// <summary>
        /// Paints the name of the day columns when Calendar.DaysMode is <see cref="DaysModes.Short"/>
        /// </summary>
        /// <param name="e">Paint Info</param>
        public virtual void OnDrawDayNameHeaders(CalendarRendererEventArgs e)
        {
            ...
            {
                DateTime atDate = startDate.AddDays(i);
        ...
        {
            Rectangle days = e.Calendar.DaysBodyRectangle; 
            days.Inflate(-1, -1);
            Region oldclip = e.Graphics.Clip;
            bool doClip = (e.Calendar.DaysMode == DaysModes.Expanded);
            bool clipped = false;
        ...
        public virtual void OnDrawItemContent(CalendarRendererItemBoundsEventArgs e)
        {
            ...
            {
                /*
                 * Step to draw in the item. 
                 * -------------------------
                 * 1. Draw Image (if have and adjust other bounds).
                 * 2. Draw Start (And End) Time.
                 * 3. Draw Item Text.
                 * 
                */
                ...
                if (e.Item.Image != null)
                {
                    ...
                    if ((e.Item.Image.Width <= tBounds.Width) && (e.Item.Image.Height <= tBounds.Height))
                        imgBounds.Size = e.Item.Image.Size;
                    else if (e.Item.Image.Width > tBounds.Width)
                    {
                        imgBounds.Width = tBounds.Width;
                        imgBounds.Height = e.Item.Image.Height * tBounds.Width / e.Item.Image.Width;
                        if (imgBounds.Height > tBounds.Height)
                            imgBounds.Height = tBounds.Height;
                    }
                    else
                    {
                        imgBounds.Width = e.Item.Image.Width * tBounds.Height / e.Item.Image.Height;
                        imgBounds.Height = tBounds.Height;
                    }
                    switch (e.Item.ImageAlign)
                    {
                        case ItemImageAligns.North:
                            tBounds.Height -= imgBounds.Height;
                            tBounds.Y += imgBounds.Height;
                            imgBounds.Y = tBounds.Y - imgBounds.Height;
                            imgBounds.X = e.Item.Bounds.X + ((e.Item.Bounds.Width - imgBounds.Width) / 2);
                            break;
                        case ItemImageAligns.South:
                            tBounds.Height -= imgBounds.Height;
                            imgBounds.Y = tBounds.Bottom;
                            imgBounds.X = e.Item.Bounds.X + ((e.Item.Bounds.Width - imgBounds.Width) / 2);
                            break;
                        case ItemImageAligns.East:
                            tBounds.Width -= imgBounds.Width;
                            imgBounds.X = tBounds.Right;
                            imgBounds.Y = e.Item.Bounds.Y + ((e.Item.Bounds.Height - imgBounds.Height) / 2);
                            break;
                        case ItemImageAligns.West:
                            marginLeftBuffer = imgBounds.Width;
                            tBounds.Width -= imgBounds.Width;
                            tBounds.X += imgBounds.Width + 2;
                            imgBounds.X = tBounds.Left - imgBounds.Width;
                            imgBounds.Y = e.Item.Bounds.Y + ((e.Item.Bounds.Height - imgBounds.Height) / 2);
                    ...
                    //evt.Bounds = tBounds;
                    OnDrawItemImage(new CalendarRendererItemBoundsEventArgs(e, imgBounds, false, false));
                }
                ...
                if (e.Item.ShowEndTime && i == rectangles.Count - 1)
                {
                    ...
                    CalendarRendererBoxEventArgs crbe = new CalendarRendererBoxEventArgs(e, rEndTime, endTime, secondaryForecolor);
                    this.OnDrawItemEndTime(crbe);
                ...
                if (e.Item.ShowStartTime && i == 0)
                {
                    ...
                    // adjust X when show image at left side.
                    rStartTime.X += marginLeftBuffer;
                    CalendarRendererBoxEventArgs crbe = new CalendarRendererBoxEventArgs(e, rStartTime, startTime, secondaryForecolor);
                    crbe.Tag = e.Item.StartDate;

                    this.OnDrawItemStartTime(crbe);
                ...
                CalendarRendererBoxEventArgs evt = new CalendarRendererBoxEventArgs(e, r, e.Item.Subject,
                ...
                if (e.Item.IsOnDayTop || Calendar.DaysMode == DaysModes.Short)
                {
                ...
                // raise on item draw content.

                Calendar.OnItemDrawContent(evt);

                

                if (marginLeftBuffer > 0)
                {
                    Rectangle newBounds = evt.Bounds;
                    newBounds.X += marginLeftBuffer;
                    newBounds.Width -= marginLeftBuffer;
                    evt.Bounds = newBounds;
                }
                OnDrawItemText(evt);
            }
        }
        ...
        {
            if ( (Calendar.Weeks == null) || !Calendar.ShowWeekHeader)
                return;
            ...
            for (int i = 0; i < Calendar.Weeks.Length; i++)
            {
                ...
                this.OnDrawWeekHeader(new CalendarRendererBoxEventArgs(e,
                    Calendar.Weeks[i].HeaderBounds, 
                    str, TextFormatFlags.Default));
            }
        }
